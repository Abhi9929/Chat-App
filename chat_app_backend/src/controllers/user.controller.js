...
const userschema = zod.object({
  firstName: zod.string().min(1,{message: 'name must be 1 character'}),
  lastName: zod.string().optional(),
  email: zod.string().email({message: 'Invalid credentials'}),
  password: zod.string().min(6, {message: 'password must be of 6 characters'}),
});
...
const signUpUser = asyncHandler(async (req, res) => {
  ...
  if (!response.success) {
    throw new ApiError(411, "Invalid user credentials");
  }
